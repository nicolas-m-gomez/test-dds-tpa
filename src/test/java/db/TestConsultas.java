...
public class TestConsultas {
	...
	public void testObtenerCategoriaONGTest() {
		assertEquals("ONG Test", em.find(Categoria.class, "ONG Test").getNombre());
	}
	
	@Test
	public void testObtenerDireccionCalleFalsa () {
		assertEquals("Calle Falsa", em.createNativeQuery("SELECT direccion_calle FROM direcciones WHERE direccion_calle = \'Calle Falsa\'").getSingleResult().toString());
	}
	
	@Test
	public void testObtenerItemsDeBD() {
		assertEquals("computadora",em.createNativeQuery("SELECT descripcion FROM Items WHERE descripcion = \'computadora\'").getSingleResult().toString());
		assertEquals("impresora",em.createNativeQuery("SELECT descripcion FROM Items WHERE descripcion = \'impresora\'").getSingleResult().toString());
		assertEquals("silla",em.createNativeQuery("SELECT descripcion FROM Items WHERE descripcion = \'silla\'").getSingleResult().toString());
	}
	
	@Test
	public void testObtenerEgresoDeBD() {
		assertEquals("2020-05-15",em.createNativeQuery("SELECT CAST(fecha as char) FROM Egresos WHERE fecha = \'20200515\'").getSingleResult().toString());
	} 
